...
if (!$pacbio_seq_file || !$read_fastq_dir || !$read_type || !$single_reads){
    print "Usage: \n";
    print "perl $0 \n";
    print "     -r PacBio sequence file (in FASTA format) \n";
...
# Step 2: concatnate all PacBio sequences into one large sequence
my $ref_seq_file = "pacbio_concatnated.fasta";
&concatnate_pacbio_seqs($pacbio_seq_file, $ref_seq_file);
...
while(my $read_fastq_file = readdir(DIR)) {
	...
	my $out_sai_file = $read_fastq_file ."_aln_sa.sai";
	my $out_sai_file1 = $read_fastq_file ."_aln_sa_1.sai";
	my $out_sai_file2 = $read_fastq_file ."_aln_sa_2.sai";
	my $out_sam_file = $read_fastq_file ."_aln_sa.sam";
	
	my $gap_open = 10;
	my $gap_extension = 6;
	my $long_deletion_witin = 3;
	my $indel_within = 3;
	...
	if ($read_type eq 'solid') {
		...
		if ($single_reads eq 's') {
			if ($file =~ /\.fastq$/ || $file =~ /\.txt$/) {
				$cmd = "$bwa aln -t$threads -o $gap_open -e $gap_extension -q 20 -d $long_deletion_witin -i $indel_within $ref_seq_file $file > $out_sai_file";
			} elsif ($file =~ /\.bam$/) {
				$cmd = "$bwa aln -t$threads -o $gap_open -e $gap_extension -q 20 -d $long_deletion_witin -i $indel_within $ref_seq_file $file > $out_sai_file";
			}
			print "system \"$cmd\"\n";
			...
			if ($file =~ /\.bam$/) {
				$cmd = "$bwa aln -t$threads -o $gap_open -e $gap_extension -q 20 -d $long_deletion_witin -i $indel_within $ref_seq_file -b1 $file > $out_sai_file1";
				print "system \"$cmd\"\n";
				system $cmd;
				$cmd = "$bwa aln -t$threads -o $gap_open -e $gap_extension -q 20 -d $long_deletion_witin -i $indel_within $ref_seq_file -b2 $file > $out_sai_file2";
				print "system \"$cmd\"\n";
				system $cmd;
				...
				$cmd = "$bwa aln -t$threads -o $gap_open -e $gap_extension -q 20 -d $long_deletion_witin -i $indel_within $ref_seq_file $forward_file > $out_sai_file1";
				print "system \"$cmd\"\n";
				system $cmd;
				$cmd = "$bwa aln -t$threads -o $gap_open -e $gap_extension -q 20 -d $long_deletion_witin -i $indel_within $ref_seq_file $reverse_file > $out_sai_file2";
				print "system \"$cmd\"\n";
				system $cmd;
    ...
    # Step 4a: calculate the statistics for each analying bam file

	my $stat_summary_file = $out_sam_file . "_stats.txt";
    print "\nCalculate mapping summary statistics ...\n";
	$cmd = "$samtools flagstat $bam_file > $stat_summary_file";
	print "system \"$cmd\"\n";
    system $cmd;
    ...
    # Step 8: Generate consensus file
    my $consensus_pileup_file = $sorted_bam_file . "_consensus.pileup";
    print "\nGenerating consensus file ...\n";
    $cmd = "$samtools pileup -cf $ref_seq_file $sorted_bam_file" . ".bam > $consensus_pileup_file";
    print "system \"$cmd\"\n";
    system $cmd;
...
#    if (-e $$consensus_pileup_file) {
#		my $file_size = -s $consensus_pileup_file;
#		if ($file_size > 10000 && $forward_file && $reverse_file) {
#		    unlink $forward_file;
#		    unlink $reverse_file;
#		}
#    }
...
sub concatnate_pacbio_seqs {
	my ($seq_file, $concatnated_file) = @_;
    open (SEQ, "<$seq_file") or die ("Can not open the file $seq_file: $!\n");
    ...
    my $name = '';
    my $count = 0;
    my $concat_seq_count = 0;
	
	open (OUT, ">$concatnated_file") or die ("Can not open the file $concatnated_file: $!\n");
    ...
    while (my $line = <SEQ>) {
		...
		if ($line =~ /^>(\S+)/) {
			...
			$count++;
	 	    ...
	 	    if ($seq ne '' && $name ne $pre_name && $pre_name ne '') {
				if (length($one_seq) >= 2000000) {
					$concat_seq_count++;
					$one_seq .= $seq;
					print OUT ">pacbio-" . "$concat_seq_count\n";
				    print OUT &format_seq($one_seq, 70);
					$one_seq = '';
				} else {
					$one_seq .=  $seq . 'NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN';
				}
#print "$one_seq\n";
